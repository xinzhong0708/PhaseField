function [E, mu] = PF_IMEX_Solver2D_Diffusion_MuOnly_Periodic(M, kappa, dx, dy, nx, ny, E, mu, dt_phy, Chi, s, chiRelFloor, chiAbsFloor)
...
% Periodic indexing
perI = @(i,sh) periodic_index(i + sh, ny);
perJ = @(j,sh) periodic_index(j + sh, nx);

jjL   = perJ(jj,-1);  jjR   = perJ(jj,+1);
jjL2  = perJ(jj,-2);  jjR2  = perJ(jj,+2);
iiU   = perI(ii,-1);  iiD   = perI(ii,+1);
iiU2  = perI(ii,-2);  iiD2  = perI(ii,+2);

iiUR  = perI(ii,-1);  jjUR  = perJ(jj,+1);
iiDR  = perI(ii,+1);  jjDR  = perJ(jj,+1);
iiUL  = perI(ii,-1);  jjUL  = perJ(jj,-1);
iiDL  = perI(ii,+1);  jjDL  = perJ(jj,-1);
...
function idx = periodic_index(idx, n)
% Periodic indexing:
% ... n-1 n | 1 2 ... n-1 n | 1 2 ...
idx = mod(idx - 1, n) + 1;
end