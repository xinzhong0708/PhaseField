%% Create_Map2D_Bands_PT_Theta.m
% Generate Map2d.mat for pseudo-1D phase bands.
%
...
%   tl_g0(T,P,...)
%
% Each band is one grain and has an orientation:
%
...
phs_name = {'Clinopyroxene','Olivine','Garnet','Feldspar','Quartz'};

% Requested phase proportions
phase_prop = [0.3 0.2 0.03 0.2 0.1];
phase_prop = phase_prop/sum(phase_prop);

% Initial independent endmember compositions
c_value = cell(1,numel(phs_name));
c_value{1} = [0.05 0.15 0 0.04 0.4];      % Clinopyroxene
c_value{2} = [0.00 0.45 0.45];            % Olivine
c_value{3} = [0.45 0.45];                 % Garnet
...
% [] means zero orientation for all bands.
% To set manually, use a row vector with length Ngrain after band_order.
theta_grain_user = [];
...
fprintf('Initial pseudo-1D phase bands\n')
