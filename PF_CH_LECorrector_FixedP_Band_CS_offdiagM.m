...
for l = 1:Ne
    ...
    % --------------------------------------------------------
    % Diffusion operator D_lm on mu_m.
    %
    % Off-diagonal mobility is included only in this second-order
    % Laplacian part.  The fourth-order kappa term below still uses
    % the diagonal mobility M_ll through Ml.
    % --------------------------------------------------------
    Ml   = Get_Mobility_Local(PARAM,l,l,ny,nx);

    D_muit = zeros(size(idx_c));
    Dcoef  = cell(Ne,5);
    ...
    % --------------------------------------------------------
    % D_lm * dmu_m.  Off-diagonal M only enters here.
    % --------------------------------------------------------
    for m = 1:Ne
...
use_full_M = iscell(PARAM.M) && size(PARAM.M,1) >= l && ...
    size(PARAM.M,2) >= m && size(PARAM.M,1) > 1 && size(PARAM.M,2) > 1;
