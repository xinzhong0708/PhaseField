...
% ideal mixing term
% Melt(H18) is a special Temkin melt in Thermolab:
%   [mtpl,z,zt] = temkin_H18(...)
% which rescales z, evaluates a composition-dependent mtpl, and sets
% zt = ones(size(zt)). Therefore p*(zt.*log(zt)) = 0 for this phase.
if IsTemkinH18(pars)
    [z,mtpl_eff] = TemkinH18_Z_Mtpl(pars,c_realt);

    zlogz = z .* log(z + double(z==0));
    g_id  = RT * sum(mtpl_eff .* zlogz, 2);
else
    % regularized ideal mixing term for all normal solution phases
    z     = c_realt * zt + 1e-30;
...
%   eval(mtpl)
%
% For Melt(H18), Thermolab then sets zt = ones(size(zt)), so the
% p*(zt.*log(zt)) subtraction in the ideal entropy term is exactly zero.
...
% Make mtpl compatible with z for vectorized evaluation.
