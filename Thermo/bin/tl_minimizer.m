function [alph,Npc,pc_id,p,g] = tl_minimizer(T,P,Nsys,phs_name,p,td,options,rho_w,eps_di,g0,v0)
...
for i_ref = 1:nref    
    ...
    %Refine
    % for iii = 1:size(p{1},2)
    %     A(:,iii) = linspace(p{1}(1,iii),p{1}(end,iii),2000);
    % end
    % p{1} = A;
    if disp_ref == 1
