clear,figure(3);clf;addpath ./bin ./Utilities/ ./Solutions/ ./EOS
%Energy scaling
E_sc          =  5e4;
%Choose P-T
T             = [1400  1400  ] + 273.15;
...
%Final result
f_Melt        =  f;  % energy
S_Melt        =  S;  % suscetibility
mu_Melt       =  mu; % mu
H_Melt        =  H;  % H
n_Melt        =  td.n_em(:,1:end-1)./repmat( sum(td.n_em(:,1:end-1),2) , 1 ,size(td.n_em(:,1:end-1),2)  );
c_Melt        = {'Fe','Mg','Al','Ca','Si'};

save('Data_Melt','f_Melt','S_Melt','mu_Melt','c_Melt','H_Melt','n_Melt')
