function [STATE_NEW,DIAG] = PF_Coupled_ACCH_LETangent_ACOnly_CS(STATE_REF,PARAM,MODEL,GRID,PHYS,NUM,STATE_COEF)
%PF_COUPLED_ACCH_LETANGENT_ACONLY_CS Fine-grid AC predictor only.
%
% Purpose:
%   This is the clean split predictor for the coarse/fine CH scheme.
%   It updates phi on the fine grid, but does NOT solve global dmu here.
%   The chemical field is corrected afterwards by
%       PF_CH_LECorrector_FixedP_CoarseFine_Band_CS
%
% Update performed here:
%   1) solve implicit Allen-Cahn on the fine grid with dmu = 0;
%   2) update phi and p;
%   3) recover E from the phase-fraction change only:
%          E_new = E_old + sum_g e_g*(p_new_g - p_old_g)
%
% This avoids the unstable mismatch caused by combining fine-grid dphi with
% a coarse-grid dmu inside the same predictor.
%
% Optional controls:
%   NUM.CM_dphi_cap     cap max |dphi| in this predictor, optional
%   NUM.norm_phi        normalize phi after update, existing flag
%   NUM.cut_phi         cut phi to [0,1], existing flag
...
% Optional AC increment cap for robustness.
...
% ------------------------------------------------------------
STATE_NEW       = STATE_REF;
STATE_NEW.phi   = STATE_REF.phi + dphi;
...
% ------------------------------------------------------------
% Recover E from fine-grid phase motion only.
% The following CH corrector supplies the chemical dmu part.
% ------------------------------------------------------------
e0 = STATE_COEF.e;
STATE_NEW.E = STATE_REF.E;

for ie = 1:Ne
    En = STATE_REF.E{ie};
    for ig = 1:Ngrain
        dp_ig = STATE_NEW.p(:,:,ig) - STATE_REF.p(:,:,ig);
        En = En + e0{ig}{ie} .* dp_ig;
    end
    STATE_NEW.E{ie} = En;
end

% Retain coefficient fields. The next LE_Run overwrites them.
STATE_NEW.chi = STATE_COEF.chi;
STATE_NEW.e   = STATE_COEF.e;
