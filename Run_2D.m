...
rng(1)
% md                 =  GRID.nx/2;
% STATE.c{1}{1}      =  STATE.c{1}{1}.* 1+0.001*(rand(size(STATE.c{1}{1}))-0.5);
% STATE.c{1}{2}      =  STATE.c{1}{2}.* 1+0.001*(rand(size(STATE.c{1}{2}))-0.5);
% STATE.c{1}{3}      =  STATE.c{1}{3}.* 1+0.001*(rand(size(STATE.c{1}{3}))-0.5);
...
PHYS.sigma         =  0.2/(PHYS.E_sc*PHYS.L_sc);                           % surface energy (J/m^2)
...
for it = 1:1e6
    ...
    % STATE_TRIAL          =    LE_Run_Mode(STATE_CORR,PARAM,MODEL);
    % 
    % % TIME STEP UPDATE
    ...
    % %DIRECT COUPLED SOLVER WITH GP PREDICTOR RESPONSE AND LE FINAL STATE
    STATE_OLD = STATE;
    ...
    % Smooth predictor thermodynamics only
    PARAM.LE_mode       = 'GP';
    PARAM.LE_GP_write_E = false;
    PARAM.LE_iter_GP    = [100 100 100 100];
    PARAM.LE_alpha      = [0.6 0.4 0.3 0.2];

    STATE_LE0 = LE_Run_Mode(STATE_RAW,PARAM,MODEL);

    % Make sure GP does not change conserved E
    STATE_LE0.E = STATE_RAW.E;

    % Fixed-p chemical corrector
    STATE_CORR = PF_CH_LECorrector_FixedP_Band(STATE_OLD,STATE_LE0,PARAM,GRID,PHYS,NUM);

    % Final state must be LE, not GP
    PARAM.LE_mode        = 'GP';
    PARAM.LE_GP_write_E  = false;
    PARAM.LE_alpha       = [0.6 0.4 0.3  0.2 ];
    PARAM.LE_iter_LE     = [100 500 1000 1000];
    ...
    %     PF_Plot([2,2,4],'c11',STATE,GRID,MODEL,TIME,DTPHY,PHASE)
    %     drawnow
    % end
